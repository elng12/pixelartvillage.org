...
| --- | --- | --- | --- | --- |
| 建议 | 构建/样式 | scripts/build/inline-critical-css.cjs:53 | Critters 内联阶段重复出现选择器解析告警：“.group-open\:rotate-45:is(:where(.group):is([open],) *) -> Empty sub-selector”。影响日志可读性、可能造成个别页面关键 CSS 未内联。 | 短期对该告警去重/降噪；中期定位产物来源（Tailwind 变体/第三方产物）并在源码端修复或剔除无效类。 |
...
| 建议 | 构建可控 | scripts/build/inline-critical-css.cjs:1–60 | 内联器对问题选择器重复告警。 | 包一层日志管道对相同告警去重/分级；必要时对产物进行预扫描并过滤。 |
...
- 关键文件引用（便于复核）：
  ...
  - scripts/build/inline-critical-css.cjs:53（Critters 处理日志）
