...
test.describe('Static pages visibility', () => {
  ...
  test('Contact page renders content', async ({ page }) => {
    ...
    await expect(page.getByTestId('contact-feedback-link')).toHaveAttribute('href', 'https://docs.google.com/forms/d/e/1FAIpQLSdjw5_Q-QmCChechdq3bjsxoSzQ5kYgnyCZ7tpIMSND6CbniA/viewform?usp=header')
    await expect(page.getByTestId('footer-contact-link')).toHaveAttribute('href', '/contact/')
    await expect(page.getByTestId('contact-email-text')).toHaveText('2296744453m at gmail dot com')
    await expect(page.getByTestId('footer-email-text')).toHaveText('2296744453m at gmail dot com')
    await expect(page.locator('a[href^="mailto:"]')).toHaveCount(0)
    await expect(page.locator('text=/2296744453m@gmail\\.com/i')).toHaveCount(0)
